...
bool BattlegroundQueue::FillXPlayersToBG(BattlegroundBracketId bracket_id, Battleground* bg, bool start)
{
	...
	if (sWorld->getBoolConfig(BATTLEGROUND_CROSSFACTION_ENABLED) && (sBattlegroundMgr->isTesting() || queuedPeople >= bg->GetMinPlayersPerTeam() * 2 || !start))
	{
		...
		for (QueuedGroupMap::reverse_iterator itr = m_QueuedGroupMap.rbegin(); itr != m_QueuedGroupMap.rend(); ++itr) {
			...
			if (ginfo->IsInvitedToBGInstanceGUID || std::find(allypool.begin(), allypool.end(), ginfo) != allypool.end()
			...
			if (start) {
				if (bg->GetSwitchTeam()) {
					...
					bg->m_AlliancePlayersCount++;
				} 
				else {
					itr->second->teamId = TEAM_HORDE;
					bg->m_HordePlayersCount++;
				}
				
				if (bg->GetAlliancePlayersCount() >= bg->GetHordePlayersCount())
					bg->setSwitchTeam(false);
				else bg->setSwitchTeam(true);
			}
			else {
				if (bg->GetHordePlayersCount() >= bg->GetAlliancePlayersCount())
					itr->second->teamId = TEAM_ALLIANCE;
				else itr->second->teamId = TEAM_HORDE;
			}
